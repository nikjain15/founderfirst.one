...
insert into auth.users (id, email, aud, role) values
  ('00000000-0000-0000-0000-00000000c0aa', 'ownerCU@test.dev',   'authenticated', 'authenticated'),
  ('00000000-0000-0000-0000-00000000c0ee', 'nonmemberCU@test.dev','authenticated', 'authenticated');

insert into organizations (id, type, name, created_by) values
  ('00000000-0000-0000-0000-00000000c0b1', 'business', 'Catch-up Biz', '00000000-0000-0000-0000-00000000c0aa');
insert into memberships (user_id, org_id, role, status) values
  ('00000000-0000-0000-0000-00000000c0aa', '00000000-0000-0000-0000-00000000c0b1', 'owner', 'active');
insert into org_accounting_settings (org_id, home_currency) values
  ('00000000-0000-0000-0000-00000000c0b1', 'USD')
...
create temp table _e23a as select * from post_journal_entry(
  '00000000-0000-0000-0000-00000000c0aa', '00000000-0000-0000-0000-00000000c0b1', '2023-03-01', 'k-23a',
  '[{"account_id":"00000000-0000-0000-0000-00000000c102","amount_minor":5000,"side":"D"},
    {"account_id":"00000000-0000-0000-0000-00000000c101","amount_minor":5000,"side":"C"}]'::jsonb,
  'manual', null, 'ADOBE *2023');
create temp table _e23b as select * from post_journal_entry(
  '00000000-0000-0000-0000-00000000c0aa', '00000000-0000-0000-0000-00000000c0b1', '2023-07-01', 'k-23b',
  '[{"account_id":"00000000-0000-0000-0000-00000000c102","amount_minor":3000,"side":"D"},
    {"account_id":"00000000-0000-0000-0000-00000000c101","amount_minor":3000,"side":"C"}]'::jsonb,
  'manual', null, 'MYSTERY CHARGE');
create temp table _e24 as select * from post_journal_entry(
  '00000000-0000-0000-0000-00000000c0aa', '00000000-0000-0000-0000-00000000c0b1', '2024-02-01', 'k-24',
  '[{"account_id":"00000000-0000-0000-0000-00000000c102","amount_minor":9000,"side":"D"},
    {"account_id":"00000000-0000-0000-0000-00000000c101","amount_minor":9000,"side":"C"}]'::jsonb,
...
create temp table _ba as select catch_up_batch_approve(
  ...
  jsonb_build_array(
    jsonb_build_object('entry_id', (select id from _e23a), 'to_account_id', '00000000-0000-0000-0000-00000000c103', 'confidence', 0.9,  'learn_value', 'adobe'),
    jsonb_build_object('entry_id', (select id from _e23b), 'to_account_id', '00000000-0000-0000-0000-00000000c103', 'confidence', 0.3,  'learn_value', 'mystery')
  )
) as r;
...
select is(
  (select coalesce(sum(case side when 'D' then amount_minor else -amount_minor end),0)::int
     from journal_lines where account_id = '00000000-0000-0000-0000-00000000c103'),
...
select throws_ok($$
  select catch_up_batch_approve('00000000-0000-0000-0000-00000000c0ee','00000000-0000-0000-0000-00000000c0b1',
    '[]'::jsonb)
...
select close_accounting_period('00000000-0000-0000-0000-00000000c0aa','00000000-0000-0000-0000-00000000c0b1',
...
create temp table _ba24 as select catch_up_batch_approve(
  ...
  jsonb_build_array(jsonb_build_object('entry_id', (select id from _e24),
    'to_account_id', '00000000-0000-0000-0000-00000000c103', 'confidence', 0.95, 'learn_value', 'adobe'))
) as r;
...
select jsonb_array_elements(catch_up_progress(
  '00000000-0000-0000-0000-00000000c0aa', '00000000-0000-0000-0000-00000000c0b1')) as y;
