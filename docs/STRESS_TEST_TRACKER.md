...
approves at three points only; everything else is handled by the build process.
...
**Coverage grows one feature at a time, or fanned across many:**
...
1. **Merge/deploy wave** - approve or run the integrator's batched merge+deploy. Can
   graduate to standing authorization once the process earns trust (rollback always one
