...
| **Security engineer / pentester** | The RLS model, the RPC write path, token storage, and a dependency posture | Whether this is safe to point at a production Plaid connection | S-1 is now a gate rather than an assertion, and all three npm trees are at **0 unwaived advisories at any severity**, low included, down from 37 in the workspace on 2 Aug. One waiver remains, `react-router` `GHSA-qwww-vcr4-c8h2`, which the advisory itself scopes to the unstable RSC APIs that neither SPA imports. The Astro waiver is gone because the upgrade was taken. S-2 (Plaid) and S-7 (Xero) are both closed, so no connector writes a plaintext token. Secret scanning and CodeQL run in CI. **What a pentester would still find:** the gate covers npm and not Python, so `torch==2.6.0` in the two Fly TTS servers sits against 8 open advisories that nothing in CI will ever fail on; no eslint or repo-wide typecheck; no `dependabot.yml`; and no alerting on anything. Plaid's own production access review is a separate, real gate we have not approached |
