...
export async function waitForFounderReady(page) {
  ...
  await expect(page.locator(".tab-bar, nav[aria-label='Primary navigation']").first()).toBeVisible({ timeout: 15_000 });
}

export async function waitForCpaReady(page) {
  await page.waitForLoadState("domcontentloaded");
  // Wait for either the CPA shell (with "Penny" wordmark) or auth gate
  await expect(
    page.locator("text=Penny").first()
