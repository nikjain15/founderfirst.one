...
insert into auth.users (id, email, aud, role) values
  ('00000000-0000-0000-0000-0000000p0a01', 'pOwner@test.dev', 'authenticated', 'authenticated'),
  ('00000000-0000-0000-0000-0000000p0d02', 'pOther@test.dev', 'authenticated', 'authenticated');

insert into organizations (id, type, name, created_by) values
  ('00000000-0000-0000-0000-0000000p0b01', 'business', 'P Biz',   '00000000-0000-0000-0000-0000000p0a01'),
  ('00000000-0000-0000-0000-0000000p0b02', 'business', 'P Other', '00000000-0000-0000-0000-0000000p0d02');

insert into memberships (user_id, org_id, role, status) values
  ('00000000-0000-0000-0000-0000000p0a01', '00000000-0000-0000-0000-0000000p0b01', 'owner', 'active'),
  ('00000000-0000-0000-0000-0000000p0d02', '00000000-0000-0000-0000-0000000p0b02', 'owner', 'active');

insert into org_accounting_settings (org_id, home_currency) values
  ('00000000-0000-0000-0000-0000000p0b01', 'USD')
...
insert into external_connections (id, org_id, provider, realm_id, tenant_name, access_token, status, connected_by) values
  ('00000000-0000-0000-0000-0000000pc001', '00000000-0000-0000-0000-0000000p0b01', 'plaid',
   'item-abc', 'Test Bank', 'access-sandbox-xyz', 'active', '00000000-0000-0000-0000-0000000p0a01');
...
select lives_ok($$
  select plaid_ingest_transactions(
    '00000000-0000-0000-0000-0000000p0a01', '00000000-0000-0000-0000-0000000p0b01',
    '00000000-0000-0000-0000-0000000pc001',
...
select is((select count(*)::int from bank_transactions where org_id='00000000-0000-0000-0000-0000000p0b01'),
  2, 'two bank_transactions rows created');
select is((select count(distinct journal_entry_id)::int from bank_transactions where org_id='00000000-0000-0000-0000-0000000p0b01'),
...
select is(
  (select coalesce(sum(case when side='D' then amount_minor else -amount_minor end),0)
   from journal_lines where org_id='00000000-0000-0000-0000-0000000p0b01'),
...
select is((select count(*)::int from journal_entries where org_id='00000000-0000-0000-0000-0000000p0b01' and source='import'),
...
select is(
  (select je.status::text from journal_entries je
   where je.org_id='00000000-0000-0000-0000-0000000p0b01' and je.idempotency_key='ext:plaid:t1'),
...
select is(
  (select bt.amount_minor from bank_transactions bt
   where bt.org_id='00000000-0000-0000-0000-0000000p0b01' and bt.plaid_transaction_id='t1'),
...
select is(
  (select state::text from bank_transactions where org_id='00000000-0000-0000-0000-0000000p0b01' and plaid_transaction_id='t2'),
  'removed', 'removed: t2 row state = removed (never deleted)');
select is(
  (select je.status::text from journal_entries je
   where je.org_id='00000000-0000-0000-0000-0000000p0b01' and je.idempotency_key='ext:plaid:t2'),
  'reversed', 'removed: t2 original entry reversed');

-- ── 6. tenant scope: the other org's owner cannot ingest into P Biz's item ────
select throws_ok($$
  select plaid_ingest_transactions(
    '00000000-0000-0000-0000-0000000p0d02', '00000000-0000-0000-0000-0000000p0b01',
    '00000000-0000-0000-0000-0000000pc001',
    '[{"transaction_id":"tX","date":"2026-07-01","amount_minor":-100,"name":"X"}]'::jsonb,
    '[]'::jsonb, '[]'::jsonb)
$$, 'forbidden: actor may not write org 00000000-0000-0000-0000-0000000p0b01',
