...
The passwordless sign-in email is generated by **Supabase Auth**, not our code — its
template lives in the Supabase dashboard (Authentication → Email Templates) and does
**not** go through `emailShell()`. To keep it on-brand, mirror the shell's structure
(eyebrow, one-sentence heading, single pill CTA, muted footer) into that template by
hand. See [`EMAIL_REGISTRY.md`](EMAIL_REGISTRY.md) entry 4.
