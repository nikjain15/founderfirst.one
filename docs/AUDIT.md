...
| IQ-1-CLEANUP | Extends the IQ-1 row: now that the encrypt/decrypt roundtrip is proven on live prod crypto, `_iq1_cleanup_legacy_qbo_plaintext()` nulls every remaining plaintext `access_token`/`refresh_token` (defensively encrypting first if a row somehow still lacks its `_enc` value) — no plaintext token is left readable/populated post-cleanup; columns are kept (rollback-safe, nothing dropped) | pgTAP `iq1_cleanup_qbo_plaintext_test.sql` (9: nulls an already-encrypted row's plaintext · `ext_connection_secrets` still decrypts it post-cleanup · defensively encrypts+nulls a never-encrypted row and decrypts it too · tokenless row untouched · service_role-only) | ⬜ red-teamed; pgTAP runs in CI (no local Docker) — verify before merge |
...
| Owner/CPA · 1099-NEC e-file — TaxBandits spike (efile-1099, EFILE-A1) | ⬜ untested → stress pass | SPIKE (prove the path, not GA): maps the EXISTING W2.5 1099-NEC roll-up (`ninetynine_nec_summary`) → the TaxBandits 1099-NEC Create/Transmit payload (`_shared/taxbandits.ts`) behind the FULL TRUST GATE. `efile-1099` fn ops: `preview` (pure mapping, who's included/below-threshold/not-ready — no provider call), `tin_match` (pre-check; a mismatch is surfaced and BLOCKS transmit), `transmit` (requires `confirm:true` AND a prior no-mismatch TIN-match recorded in the log for EVERY included vendor — NEVER auto-transmits), `history` (read the immutable log). ⛔ NO FAKE SUCCESS: absent `TAXBANDITS_*` creds → the transmit path returns a DRY-RUN mapped-payload preview (`phase='dry_run'`, logged), never a synthesized accept; a provider reject/error ack is stored verbatim (`Errors[]` preserved) and surfaced, never swallowed; `getAccessToken` fails loud on a non-200. NO duplicate vendor store (reads W2.5 vendors; full TIN/EIN supplied at confirm time is used transiently and NEVER persisted — log keeps last-4 only). Immutable append-only log: migration `20260708030000_efile_a1_1099_submission_log.sql` — `efile_submissions` UPDATE/DELETE raise (`restrict_violation`; service_role has INSERT grant only), data-layer trust-gate triggers enforce transmit-needs-confirmer + dry_run-can't-be-accepted; write RPC `efile_record_event` service_role-only, actor-first, `can_write_org_as`-gated, `ledger_audit` inline. pgTAP `efile_a1_1099_submission_log_test.sql` (16 assertions: record+audit, confirm-gate, no-fake-success, reject-ingest, immutability, service_role-only, no-dup-store); deno `_shared/taxbandits.test.ts` (19: payload maps from the roll-up, below-threshold skip, not-ready flags, TIN-match gating, reject classification never→success, no-creds→null=dry-run, auth JWS + fail-loud token exchange — all network-free/mocked). ⚠️ sandbox creds NOT yet provisioned (Nik human step) → live transmit unverified; spike proves mapping + gate only. |
...
| Owner + CPA · Security (`/security`, SEC-1) | ⬜ untested → stress pass | new: personal two-factor authentication — TOTP enrol/challenge/disable runs entirely against Supabase Auth's native factor API (no schema of ours); one-time recovery codes are our own hashed store (`mfa_recovery_codes`/`security_audit`, `mfa` edge fn, Admin-API factor clear on redeem). Login-time step-up (`MfaChallengeGate`, wraps every authed route) blocks any account with a verified factor until aal2; per-org "require two-factor" policy (`MfaRequiredSetting` in Settings) blocks org access via `OrgMfaGate` until the member enrols — Settings/Security stay reachable so a blocked member can fix it. pgTAP `sec1_mfa_test.sql` (27 assertions) proves one-time-use + cross-user isolation + service_role-only RPCs + (SEC-1-CPACLOSE) `org_requires_mfa` keys on a **firm** org's own flag; Vitest proves both gate decision functions pure. **Server-side write gates** reuse `_shared/mfaGate.ts` (`aalFromJwt`+`mfaSatisfied`) across 11 org-scoped write fns — incl. `cpa-close` batch-close, gated on the CPA firm user's OWN firm-org policy (aal1 on an MFA-required firm → 403 `mfa_required` BEFORE any per-client write, no partial batch; non-MFA firms unaffected; aal read from the verified JWT, never a body actor — deno `cpa-close/index.test.ts`, 5 cases). UI responsive/axe pass on the width ladder still pending. |
...
| filing-export (RV2-A2) | Structured per-suite tax export (2nd leg of the filing mission after RV2-A1). Re-shapes the tied-out RV2-A1 worksheet into the format tax software imports — generic mapped-TB CSV, Drake + UltraTax TB-import files, print-ready package — so the CPA re-keys nothing into Drake/Lacerte/ProConnect. Per-suite line codes come from **seeded** `tax_form_lines.export_codes` (DATA, never inlined); serializers are per-suite config off the pluggable registry. Export is **gated on the return being review-ready AND tying out** (an unmapped/non-tying return is never handed to tax software — the #1 filing trust risk). No re-computation: an exported line total IS the worksheet line total by construction. **Audit-log gap closed:** the export had no audit trail (report-export already covers every other export — TB/P&L/BS/GL/CF/NEC/pkg); `report-export` gains a `tax_export` report kind (+ an `html` format, since `generic_pdf` emits a real print-ready `.html`, never a true `.pdf`) and Filing's export button now fire-and-forgets a `logTaxExport` call recording who/suite/form/tax-year/filename, mirroring the W1.2 `logReportExport` pattern exactly. | `apps/app/src/tax/taxExport.test.ts` (REG `RV2A2-EXPORT-ROUNDTRIP`: build worksheet from ledger → serialize every suite → **parse the file back** → reconstructed per-line totals tie to the worksheet AND the year-scoped TB to the cent; `RV2A2-EXPORT-GATE`: unmapped-blocked · non-tying-blocked · empty-ledger · wrong-year-scoping · rounding · seeded codeMap · unknown-suite guard · filename) + existing `engine.test.ts` serializer golden strings + `supabase/functions/report-export/index.test.ts` (new: `tax_export`/`html` accepted, existing report/format kinds unchanged, sanitization never coerces/trusts shape) | ⬜ red-teamed; stress-pass pending |
