...
5. **Mint an admin JWT** for a **super/editor admin** (e.g. `<super-admin-email>`): service-role `generate_link` → `verify` (magiclink) → `access_token`. (Mutating blog RPCs gate on `is_admin_editor()`, a non-admin like `tester@` is rejected.)
