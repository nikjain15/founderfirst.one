...
-- ── 3 · gate WRITES on approval. This RE-DECLARES can_write_org_as, so it must
--        PRESERVE the current (wave3_fixes) body — membership/engagement + the
--        opt-in MFA gate (org_requires_mfa + session_is_aal2) — and only ADD the
--        approval check at the top. Dropping the MFA branch here would silently
--        un-gate MFA-required orgs (caught by sec1_mfa_test). ──────────────────
create or replace function can_write_org_as(p_actor uuid, target_org uuid)
returns boolean language plpgsql stable security definer set search_path = public as $$
declare v_ok boolean;
begin
  -- Approval gate: a pending/declined org has no write access at all.
  if not exists (
    select 1 from organizations o where o.id = target_org and o.approval_status = 'approved'
  ) then
    return false;
  end if;
  ...
  -- opt-in MFA policy: only gate orgs that enabled it (preserved from wave3_fixes).
