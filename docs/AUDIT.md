...
| **W1.3-B CPA mapping-edit UI deferred** | **RESOLVED (TAXMAP-1, 9 Jul)** — `set_account_tax_line`/`clear_account_tax_line` existed since the engine shipped but were never called from the app; a new `tax-mapping` edge fn (MFA-gated, actor-from-JWT, same ISOTEST lineage as cpa-close/cpa-collab) is the door, and `Filing.tsx`'s unmapped section now offers a CPA-only (owners view — research decision 3) line-picker + save, plus a "remove mapping" action on already-mapped override lines. Underlying RPC correctness (CPA-gate, integrity check) was already pgTAP-covered (`tax_mapping_engine_test.sql` §5/§5b); this closes the UI gap only. **Still a gap:** no live CPA-lens app-e2e walkthrough (the harness has no CPA-login fixture to extend safely without inventing one blind) — Vitest+Deno cover the logic, a live click-through is a disclosed follow-up. |
