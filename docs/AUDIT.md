...
## Coverage, how it grows with the product
...
The roadmap-v2 Wave-1 pre-deploy gate (the build-process guide rule 9): the 14-dimension rubric
+ an adversarial cross-PR stress pass, run on a **COMBINED integration branch** (not
per-PR) so failures that only appear in combination are caught before coverage ramps.
...
The Wave-4 gate (the build-process guide hard-rule #8): the 14-dimension rubric + an adversarial
...
The Wave-3 gate (the build-process guide hard-rule #8): the 14-dimension rubric + an adversarial
...
blocks. The ≤5/week budget is an explicit usability + honesty promise (the build-process guide #1,
...
⬜ by this program's rule (a row leaves ⬜ only via the formal adversarial stress program, § Coverage
