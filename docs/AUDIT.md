...
**Coverage delta:** +17 ledger rows, all ⬜ (baseline + per-PR red-team, formal stress pass pending). RV2-C1 adds the firm-level month-end close row; RV2-A2 adds the `filing-export` row (structured per-suite export round-trips + ties to worksheet/TB); SEC-2 adds the `auth-botprotect` row (Turnstile + config-driven OTP rate limit on both login forms).
