...
select is(
  (select request_count from signup_confirmation_rate_limit
    where ip_hash = encode(extensions.digest('203.0.113.9', 'sha256'), 'hex') and window_start = date_trunc('hour', now())),
...
select ok(
  (select count(*)::int from signup_confirmation_rate_limit
    where ip_hash = encode(extensions.digest('203.0.113.9', 'sha256'), 'hex')) >= 1,
