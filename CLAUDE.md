...
| Rate limit resilience + session-fresh reset (25 Apr 2026) | Root cause of "I couldn't get that right now." errors: Anthropic Tier 1 (30K Sonnet input TPM) exhausted on page load from concurrent ambient AI calls. Three fixes: (1) **Ambient calls → Haiku** — `thread.greeting`, `thread.idle`, `card.approval` now use `claude-haiku-4-5-20251001` (50K TPM, 20× cheaper); user-initiated calls (`thread.qa`, `books.qa`, `capture.parse`) stay on Sonnet. `AMBIENT_INTENTS` set in `worker-client.js`; `ambientModel` config token in `index.html`. (2) **429 backoff** — `guardrails/retry-on-fail.js` gains `RateLimitError` class + exponential backoff (8s → 16s, 2 retries); friendlier copy in `constants/copy.js`. (3) **Mount stagger** — greeting delayed 800ms, idle delayed 1.5s after queue done. (4) **Session-fresh reset** — app state moved from `localStorage` to `sessionStorage`; new tab = fresh first-time onboarding; AI response cache stays in `localStorage`. Settled decisions 22 + 23 added to `BookKeeping/demo/CLAUDE.md`. |
