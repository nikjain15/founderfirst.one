...
- **User problem / evidence.** IA-3 was explicitly **deferred until Wave 1 shipped** (BACKLOG,
  Nik 3 Jul). Wave 1 is done, so the deferral condition is met. As coverage scales and real
...
- **Impact.** Medium, operational leverage and one-source-of-truth for staff, not new customer
  value. Grows in importance as customer count and throughput grows.
...
- **User problem / evidence.** Testing has run against prod with namespaced fixtures and a
