...
| **Security engineer / pentester** | The RLS model, the RPC write path, token storage, and a dependency posture | Whether this is safe to point at a production Plaid connection | Blocks on S-P0-1 (a live `pnpm audit` today reports 1 critical and 10 high, against a rubric in our own `docs/AUDIT.md:38` that demands none) and, until IQ-2 landed, S-2 (Plaid tokens stored in plaintext). S-2 is now closed by `20260802120000_iq2_plaid_token_encryption.sql`; the equivalent Xero gap is open as S-7. Plaid's own production access review is a separate, real gate we have not approached |
...
1. **First, fix what would fail the review anyway.** S-P0-1 (dependency audit in
   CI), S-2 (encrypt Plaid tokens, now done) and S-7 (the same for Xero, still open),
   S-P0-3 (retention job plus PII minimisation
